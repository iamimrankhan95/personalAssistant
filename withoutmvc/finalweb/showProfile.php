        ...
        while ($row = mysqli_fetch_array($result)) {
            ...
            echo "
                    ...
                    <tr>
                        ...
                        <td>$row[4]</td>
                        <td >$row[5]</td>
						<td id='e'><a href='delete.php?state=profile&profileid=$row[0]'>Remove</a></td>
                        <td id='e'><a href='edit.php?state=profile&profileid=$row[0]'>Edit</a></td>
    ...
    default:
        ...
        <script>
            function search() {
                ...
                req.open("GET", "showProfile.php?state=search&q=" + searchInput.value, false);
            ...
            function search2() {
                ...
                var req = new XMLHttpRequest();
                req.open("GET", "showProfile.php?state=search2&q=" + searchInput.value, false);
                req.onreadystatechange = function () {
                    if (req.readyState == 4) {
                        var displayDiv = document.getElementById('tab');
                        displayDiv.innerHTML = req.responseText;
...
<?php		
	...
	$sql = "SELECT id, name, phone, email, address,companyname FROM profileinfo";		
	$result = mysqli_query($conn, $sql);	
					echo '<table border="1">';
					echo "<td>Id</td><td>Name<td><td>Phone</td><td>Email</td><td>Address</td><td>Company Name</td>";
					while($row = mysqli_fetch_array($result)){
						$id = $row['id'];
						$name = $row['name'];
						$phone = $row['phone'];		
						...
						echo '<tr>';
						echo "<td>$id</td><td>$name<td><td>$phone</td><td>$email</td><td>$address</td><td>$company</td>";
						echo '<tr>';
					}
					echo '</table>';
